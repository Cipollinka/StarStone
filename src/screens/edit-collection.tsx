...
>;
export const EditCollectionScreen: FC<DetailsScreenProps> = ({route}) => {
  const {collectionId} = route.params;
  const {userProfile, setUserProfile} = useUserDataProfile();

  const [collectionName, setCollectionName] = useState(
    userProfile?.collections.find(collection => collection.id === collectionId)
      ?.title || '',
  );
    ...
    if (selectedStones.includes(stoneId)) {
      return (
        <TouchableOpacity
          ...
          <Text style={{fontSize: 12, color: '#A45DFB'}}>Remove</Text>
        </TouchableOpacity>
      );
    }
    return (
